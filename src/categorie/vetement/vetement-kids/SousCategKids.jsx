...
const SousCategKids = () => {
  ...
  const { sousCategorieKids } = useParams();
  ...
  const handleFilter = async () => {
    try {
      setIsLoading(true);
      const response = await axios.get(`https://familishop.onrender.com/clothes/collection/${sousCategorieKids}/?gender=K&min_price=${min}&max_price=${max}`);
  ...
  return (
    <div className="PageContainer p-3 bg-[#E3E2E2]/40">
      <h1 className="text-[#8A8888] font-semibold text-lg mx-3 mb-2"> vetement &gt; Vêtements Kids &gt; <span className="text-[#800B8D]">{sousCategorieKids}</span></h1>
      <div className="bg-[#FFFFFF] rounded-sm">
        <div className="mx-4">
          ...
          <div className="m-2">
            <input type="text" value={min} onChange={(e) => setMin(e.target.value)} placeholder="Min" className="px-2 border-2 border-[#8A8888]/40 w-14 mx-1" /> <span className="text-[#727272]"> DA - </span>
            <input type="text" value={max} onChange={(e) => setMax(e.target.value)} placeholder="Max" className="px-2 border-2 border-[#8A8888]/40 w-14 mx-1" /> <span className="text-[#727272]"> DA </span>
                ...
                {products.map((product, i) => (
                  <div className='relative m-2 border-2 rounded-lg cursor-pointer h-[340px] hover:shadow-lg' key={product.id} onMouseEnter={() => handleMouseEnter(product.id)} onMouseLeave={handleMouseLeave}>
                    <Link className="w-[218px] h-[250px] cursor-pointer" to={'/Product/' + product.id}>
                      <img src={hoveredProductId === product.id ? product.src_image : product.alt_image} className="object-cover w-[210px] h-[250px]" alt='' />
                    </Link>
