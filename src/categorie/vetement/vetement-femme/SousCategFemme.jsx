...
const SousCategFemme = () => {
  const [isFavorited, setIsFavorited] = useState(false);
  const [min, setMin] = useState('');
  const [max, setMax] = useState('');
  const { sousCategorieFemme } = useParams();
  ...
  const handleFilter = async () => {
    try {
      const response = await axios.get(
        "https://familishop.onrender.com/clothes/collection/" +
        sousCategorieFemme +
        "/?gender=F&min_price=" +
        min +
        "&max_price=" +
  ...
  return (
    <div className="PageContainer p-3 bg-[#E3E2E2]/40 ">
      <h1 className="text-[#8A8888] font-semibold text-lg mx-3 mb-2 "> vetement <span> &gt; </span> Vêtements Femme <span> &gt; </span> <span className="text-[#800B8D]"> {sousCategorieFemme} </span></h1>
                ...
                {products.map((product, i) => (
                  <div className='relative m-2 border-2 rounded-lg cursor-pointer h-[340px] hover:shadow-lg' key={product.id} onMouseEnter={() => handleMouseEnter(product.id)} onMouseLeave={handleMouseLeave}>
                    ...
                    <Link to={'/Product/' + product.id}>
                      <h1 className='font-semibold text-[15px] px-2 mt-1'>{product.title}</h1>
                    </Link>
                    <div className='relative flex items-center justify-center'>
                      <p className='font-semibold text-xs text-[#FF385C] mx-2'>{product.price} DA</p>
                      {user ? (
                        <button onClick={handleAddToFavorites(i)} className='focus:outline-none'>
                          {product.isFavorited ? (
                        ...
                        <button
                          onClick={() => alert('Please login or create an account to add favorites')}
